...
export class ReportBuilderProcessor extends WorkerHost {
  ...
  async process(job: Job<any, any, string>): Promise<any> {
    this.logger.log(
      `Processing event on ${QUEUE_NAME}, Job with id: ${job.id} and args: ${JSON.stringify(
        job.data,
      )}`,
    );
  ...
  onActive({ id, data }: { id: string; data: number | object }) {
    this.logger.log(
      `Active event on ${QUEUE_NAME}, Job with id: ${id} and args: ${JSON.stringify(data)}`,
    );
  }
  ...
  onCompleted({ id, data }: { id: string; data: number | object }) {
    this.logger.log(
      `Completed event on ${QUEUE_NAME}, Job with id: ${id} and args: ${JSON.stringify(data)}`,
    );
  }
  ...
  onFailed({ id, data }: { id: string; data: number | object }) {
    this.logger.log(
      `Failed event on ${QUEUE_NAME}, Job with id: ${id} and args: ${JSON.stringify(data)}`,
    );
  }
}
